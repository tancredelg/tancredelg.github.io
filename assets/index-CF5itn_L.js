...
`;const n=document.querySelectorAll(".section-container"),c=document.querySelectorAll(".card-project"),l=new IntersectionObserver(s=>{s.forEach(t=>t.target.style.opacity=t.intersectionRatio)},{root:null,threshold:[0,.2,.4,.6,.8,1]}),d=new IntersectionObserver(s=>{s.forEach(t=>{t.isIntersecting?(t.target.style.opacity="1",t.target.classList.add("rounded-[2rem]")):(t.target.style.opacity="0.2",t.target.classList.remove("rounded-[2rem]"))})},{root:c[0].parentElement,rootMargin:"0px -50%",threshold:[0,.2,.4,.6,.8,1]});n.forEach(s=>l.observe(s));c.forEach(s=>d.observe(s));
